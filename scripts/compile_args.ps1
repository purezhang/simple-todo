...
Write-Host "编译器命令: $clPath $($clArgs -join ' ')"
Write-Host ""

# 使用 Start-Process 调用 cl.exe 以避免引号问题
$process = Start-Process -FilePath $clPath -ArgumentList $clArgs -NoNewWindow -Wait -PassThru -ErrorAction Stop
