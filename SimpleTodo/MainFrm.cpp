...
LRESULT CMainFrame::OnCreate(UINT, WPARAM, LPARAM, BOOL&)
{
    ...
    // 使用两次 PostMessage 延迟刷新，确保窗口已完全准备好
    PostMessage(WM_APP + 100, 0, 0);  // 第一次刷新
    SetTimer(2000, 100, nullptr);     // 100ms 后第二次强制刷新
...
void CMainFrame::ExportToTodoTxt()
{
    ...
    if (dlg.DoModal() == IDOK) {
        ...
        if (_tfopen_s(&fp, strPath, _T("w, ccs=UTF-8")) == 0 && fp) {
            // 写入注释头
            fprintf(fp, "# todo.txt Export from Simple Todo\n");
            ...
            fprintf(fp, "%s\n", time_buf);
            fprintf(fp, "# Format: (A-Z) YYYY-MM-DD Task description +project @context\n");
            fprintf(fp, "# x YYYY-MM-DD for completed tasks\n\n");
