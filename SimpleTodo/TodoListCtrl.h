...
public:
    ...
    void RefreshList() {
        if (m_pDataManager) {
            ...
            RefreshGroups();
            SetItemCountEx(m_pDataManager->GetItemCount(m_isDoneList), LVSICF_NOSCROLL);
    ...
    void RefreshGroups() {
        ...
        TCHAR szDebug[256];
        _stprintf_s(szDebug, _T("  groupIds count=%zu, isDoneList=%d\n"), groupIds.size(), m_isDoneList);
        ::OutputDebugString(szDebug);

        // 获取当前已有的分组
        int currentGroupCount = GetGroupCount();
        _stprintf_s(szDebug, _T("  currentGroupCount=%d\n"), currentGroupCount);
        ::OutputDebugString(szDebug);
        ...
        for (int groupId : groupIds) {
            ...
            for (int i = 0; i < currentGroupCount; i++) {
                if (GetGroupIdByIndex(i) == groupId) {
                    exists = true;
                    break;
                }
            }
            ...
            if (!exists) {
                ...
                _stprintf_s(szDebug, _T("  Inserting group: id=%d, header='%s'\n"), groupId, strHeader.GetString());
                ::OutputDebugString(szDebug);
                ...
                // 验证分组是否创建成功
                int newCount = GetGroupCount();
                _stprintf_s(szDebug, _T("  After insert: GetGroupCount=%d\n"), newCount);
                ::OutputDebugString(szDebug);
            } else {
